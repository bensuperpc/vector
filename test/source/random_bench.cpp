...
static void random_uint64_t(benchmark::State& state)
{
  uint64_t var = 0;
  for (auto _ : state) {
    benchmark::DoNotOptimize(var = benlib::random<uint64_t>());
...
static void random_uint64_t_min_max(benchmark::State& state)
{
...
BENCHMARK(random_uint64_t_min_max)->Name("random_uint64_t_min_max")->RangeMultiplier(64)->Range(1, 4_GB)->Threads(1);
